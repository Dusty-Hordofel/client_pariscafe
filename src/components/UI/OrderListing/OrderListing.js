...
const OrderListing = (props) => {
  ...
  const renderListing = () => {
    ...
    return (
      ...
      <>
        {contents && contents.map((dish, i) => {

          return (
            <div>
              <div className="d-flex flex-wrap mb-2 justify-content-start mt-2" key={i}>
                <div className="ml-4" style={{ padding: '5px' }}>
                  <img src={`${API_BASE_URL}/api/dishes/${dish._id}/photo`} className="dish-image" alt="Dish" />
                </div>
  ...
  return (
    <div className=" order-list col-10 col-md-9 mt-3 ml-md-2  ">
      <div className=" d-flex flex-wrap justify-content-between ml-2">
        <span style={{ fontSize: '18px', fontWeight: 'bold' }}>  Order Items ({contents && contents.length})</span>
