...
const CANCEL_STATES = [{ event: "Ordered" }, { event: "Cancelled" }];
...
export const TrackingCard = ({ status = [], refresh }) => {
  ...
  let mergedStatusList =
    status[1] === "Cancelled"
      ? _.merge(CANCEL_STATES, status)
      : _.merge(STATES, status);
  ...
  useEffect(() => {
    ...
    console.log(status);
    const event =
      status && status[statusLength - 1] && status[statusLength - 1].event;

    const eventDate =
      status && status[statusLength - 1] && status[statusLength - 1].onDate;
    ...
    <>
      {mergedStatusList.map((status, i) => (
        ...
        >
          <i
            className={
              status.onDate ? "fa fa-check-circle fa-2x" : "fa fa-clock-o fa-2x"
            }
            style={{
              marginRight: "0.2rem",
              marginLeft: "0.2rem",
              color: status.onDate ? "#570303" : "grey",
              padding: "2px",
              borderRadius: "50%",
            }}
          ></i>
          ...
          return (
            <div
              className={
                alignment === "horizontal"
                  ? "tracking-text"
                  : "tracking-text-vertical"
              }
              key={i}
            >
              <span>{status.event}</span>

              {(status.event === "Ordered" ||
              ...
              status.onDate ? (
                <>
                  ...
                  >
                    {moment(status.onDate).format("llll")}
                  </span>
                </>
              ) : null}
            </div>
  ...
  const renderHorizontal = () => {
    return (
      <div className="tracking-card d-none d-lg-block">
        <div className="tracking-header">Track Order</div>
