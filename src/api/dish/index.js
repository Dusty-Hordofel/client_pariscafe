...
export const getFilteredDishList = async (categories = []) => {
  ...
  try {
    ...
    console.log(
      "🚀 ~ file: index.js ~ line 31 ~ getFilteredDisheList ~ error",
      error
    );
    throw error;
  }
...
export const createDish = async (dish) => {
  ...
  try {
    const result = await AxiosInstance.post(`${URI}`, dish);
    return result;
  } catch (error) {
    console.log("🚀 ~ file: index.js ~ line 46 ~ createDish ~ error", error);
